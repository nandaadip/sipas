    ...
    if(empty($_SESSION['admin'])){
        ...
        <div class="container">
            ...
            <div id="lead">
                <p>Kepala Bidang</p>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT kepala_dinas, nip FROM tbl_instansi");
                list($kepala_dinas,$nip) = mysqli_fetch_array($query);
                if(!empty($kepala_dinas)){
                    echo '<p class="lead">'.$kepala_dinas.'</p>';
                } else {
                    echo '<p class="lead">, S.Kom.</p>';
                }
                if(!empty($nip)){
                    echo '<p>NIP. '.$nip.'</p>';
                } else {
